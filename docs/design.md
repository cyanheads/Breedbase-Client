...
*Ran the overall idea through my MCP design pipeline — it sketched a slightly different tool surface. Sharing in case any of it is useful to steal. Happy to prototype it if you think it's worth pursuing — otherwise just ideas to pick from.*
...
- **Goal-shaped tools as default, raw passthrough as escape hatch** — making a 1:1 API-proxy the *primary* interface tends to shift composition burden onto the LLM and produce more round trips. A documented last-resort `brapi_raw_get` / `brapi_raw_search` pair closes the completeness gap for niche endpoints (`/samples`, `/methods`, `/scales`, `/crosses`) without diluting the goal-shaped surface. Tool descriptions direct agents to `find_*`/`get_*` first; the raw tools are explicitly positioned as "when nothing else fits." This mirrors how the Plant-Phenomics-Lab/Breedbase-Client Python server operates (where `brapi_get`/`brapi_search` are the primary interface), with the raw pair positioned as an escape hatch rather than the front door.
- **Filter discovery + passthrough** — `brapi_describe_filters` (plus the paired `brapi://filters/{endpoint}` resource) lets an agent enumerate valid filters per endpoint; every `find_*` tool then accepts an `extraFilters?` map forwarded verbatim to the upstream. Goal-shaped named params cover the validated common cases; the discovery/passthrough pair handles server-specific or less-common filters without a code change. Cost: no runtime validation on passthrough values. Mitigation: preserve the full upstream 400 body so the agent can self-correct. Builds on Breedbase-Client's `get_search_parameters` idea, separating discovery (dedicated tool + resource) from usage (catch-all param on existing tools).
